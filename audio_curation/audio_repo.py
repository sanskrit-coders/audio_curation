...
class DerivativeRepo(object):
  ...
  def __init__(self, base_repo, derivative_namer, dir_name, repo_paths=None, archive_audio_item=None,
               gmusic_client=None):
  ...
  def get_files(self):
    return [self.derivative_namer(file, dir_name=self.dir_name) for file in self.base_repo.get_files() if
            os.path.exists(file)]
  ...
  def get_derived_files(self):
    ...
    """
    derived_files = [self.derivative_namer(file.file_path, dir_name=self.dir_name) for file in
                     self.base_repo.get_files() if
  ...
  def get_underived_files(self):
    logging.info(f"uds {self.__class__.__name__}")
  ...
  def delete_obsolete_derivatives(self, dry_run=False):
    ...
    repo_contents = [item for sublist in
                     [sorted(glob.glob(os.path.join(repo_path, "*"))) for repo_path in self.repo_paths] for item in
                     sublist]
  ...
  def update_derivative(self, base_file):
    logging.info(f"ud {self.__class__.__name__}")
    pass
  ...
  def is_derivative_file_outdated(self, base_file_path):
    ...
    return (not os.path.isfile(derivative_path)) or os.path.getmtime(base_file_path) >= os.path.getmtime(
      derivative_path)
...
class NormalizedRepo(DerivativeRepo):
  def __init__(self, base_repo, derivative_namer=basename_based_file_namer, archive_audio_item=None, gmusic_client=None,
               normalization_speed_multiplier=1, repo_paths=None):
    super(NormalizedRepo, self).__init__(base_repo=base_repo, derivative_namer=derivative_namer,
                                         dir_name="normalized_mp3", archive_audio_item=archive_audio_item,
                                         gmusic_client=gmusic_client, repo_paths=repo_paths)
  ...
  def update_derivative(self, base_file):
    ...
    base_mp3.save_normalized(overwrite=True, speed_multiplier=self.normalization_speed_multiplier,
                             normalized_file_path=self.derivative_namer(base_file, dir_name=self.dir_name))
...
class SpeedFileRepo(DerivativeRepo):
  def __init__(self, base_repo, derivative_namer=basename_based_file_namer, archive_audio_item=None,
    ...
    super(SpeedFileRepo, self).__init__(base_repo=base_repo, derivative_namer=derivative_namer, dir_name="speed_mp3",
                                        archive_audio_item=archive_audio_item, gmusic_client=gmusic_client,
                                        repo_paths=repo_paths)
    self.speed_multiplier = speed_multiplier

  def update_derivative(self, base_file):
    logging.info(f"ud {self.__class__.__name__}")
    mp3_utility.Mp3File(file_path=base_file, load_tags_from_file=True).speedup(speed_multiplier=self.speed_multiplier,
                                                                               out_file=self.derivative_namer(base_file,
                                                                                                              dir_name=self.dir_name))
    self.update_metadata([mp3_utility.Mp3File(file_path=self.derivative_namer(base_file, dir_name=self.dir_name),
...
class BaseAudioRepo(DerivativeRepo):
  ...
  def __init__(self, repo_paths,
    ...
    super(BaseAudioRepo, self).__init__(base_repo=None, derivative_namer=lambda x, _: x, repo_paths=repo_paths,
                                        dir_name=dir_name, archive_audio_item=archive_audio_item,
                                        gmusic_client=gmusic_client)
    self.git_repos = [_get_repo(repo_path, git_remote_origin_basepath=git_remote_origin_basepath) for repo_path in
                      repo_paths]
    ...
    if self.git_repos is not None:
      for git_repo in self.git_repos:
        changed_files.extend(
          [os.path.join(git_repo.working_tree_dir, item.a_path) for item in git_repo.index.diff(None) if
           os.path.exists(os.path.join(git_repo.working_tree_dir, item.a_path))])
      else:
        self.get_files()
  ...
  def update_git(self, collapse_history=False, dry_run=False):
    ...
    def get_changed_files(repo_x):
      ...
      changed_files.extend(
        list(filter(lambda path: os.path.basename(os.path.dirname(path)) == "mp3", repo_x.untracked_files)))
      return changed_files
    ...
    def add_changed(repo_x, dry_run=False):
      ...
      if len(changed_files) > 0:
        for fpath in changed_files:
          assert fpath.endswith(".mp3") or fpath.endswith("md") or os.path.basename(fpath) in [
            ".gitignore"] or fpath.endswith("ytdl-archive.txt"), fpath
        for fpath in changed_files:
...
class NonGitArchiveRepo(BaseAudioRepo):
  def __init__(self, repo_paths,
               archive_audio_item=None,
               git_remote_origin_basepath=None,
               gmusic_client=None):
    super(BaseAudioRepo, self).__init__(base_repo=None, derivative_namer=lambda x, _: x, repo_paths=repo_paths,
                                        dir_name="", archive_audio_item=archive_audio_item,
                                        gmusic_client=gmusic_client)
  ...
  def update_derivative(self, base_file):
    ...
    logging.info(f"ud {self.__class__.__name__}")
    if base_file.endswith("mp3"):
      mp3_file = mp3_utility.Mp3File(file_path=base_file, load_tags_from_file=True)
      # mp3_file.save_normalized(normalized_file_path=base_file, overwrite=True)
      self.update_metadata([mp3_file])
    return base_file
